// collision.ts
import { GameObject, PlayerState, Bounds, PlatformType } from '../types/game';
...
export const handlePlatformCollision = (
  ...
  platforms.forEach(platform => {
    ...
    if (checkCollision(updatedPlayer.bounds, platform.bounds)) {
      ...
      switch (direction) {
        case CollisionDirection.TOP:
          ...
          updatedPlayer.bounds.y = platform.bounds.y - updatedPlayer.bounds.height;
        ...
        case CollisionDirection.RIGHT:
          ...
          updatedPlayer.bounds.x = platform.bounds.x - updatedPlayer.bounds.width;
          updatedPlayer.velocity.x = 0;
          break;
      }
    }
    ...
    if (platform.properties?.isMoving && isPlayerOnPlatform(updatedPlayer, platform)) {
...
export const handleEnemyCollision = (
...
): { updatedPlayer: PlayerState, damageDealt: number, defeatedEnemies: Array<string> } => {
  let updatedPlayer = { ...playerState };
  let damageDealt = 0;
  ...
  enemies.forEach(enemy => {
    if (checkCollision(updatedPlayer.bounds, enemy.bounds)) {
      ...
      if (direction === CollisionDirection.BOTTOM && updatedPlayer.velocity.y > 0) {
        // Bounce off enemy
        ...
        // Apply knockback
        const knockbackDirection = direction === CollisionDirection.LEFT ? -1 : 1;
...
export const handleCollectibleCollision = (
  playerState: PlayerState,
  collectibles: Array<GameObject & { type: string; value: number; effect?: string }>
): { updatedPlayer: PlayerState, collectedItems: Array<{ id: string, type: string, value: number, effect?: string }> } => {
  let updatedPlayer = { ...playerState };
  const collectedItems: Array<{ id: string, type: string, value: number, effect?: string }> = [];
